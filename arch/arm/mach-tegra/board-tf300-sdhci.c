...
static int tf300_wifi_status_register(void (*callback)(int , void *), void *);

static int tf300_wifi_reset(int on);
static int tf300_wifi_power(int on);
static int tf300_wifi_set_carddetect(int val);

static struct wifi_platform_data tf300_wifi_control = {
	.set_power	= tf300_wifi_power,
	.set_reset	= tf300_wifi_reset,
	.set_carddetect	= tf300_wifi_set_carddetect,
};
...
static struct platform_device broadcom_wifi_device = {
	...
	.dev		= {
		.platform_data = &tf300_wifi_control,
	},
};
...
static struct tegra_sdhci_platform_data tegra_sdhci_platform_data2 = {
	.mmc_data = {
		.register_status_notify	= tf300_wifi_status_register,
#ifdef CONFIG_MMC_EMBEDDED_SDIO
...
static int tf300_wifi_status_register(
...
static int __init tf300_wifi_init(void)
{
	int rc;
...
static int __init tf300_wifi_prepower(void)
{
	if (!machine_is_tf300())
		return 0;
...
subsys_initcall_sync(tf300_wifi_prepower);
#endif

int __init tf300_sdhci_init(void)
{
