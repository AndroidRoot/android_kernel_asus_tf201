/*
 * arch/arm/mach-tegra/board-tf700-irda.c
 *
...
static int tf700_irda_mode_switch(int mode)
{
	int ret = -1;
	...
	if (ret < 0) {
		pr_err("%s: tf700_irda_tx gpio request failed %d\n",
							 __func__, ret);
	...
	if (ret) {
		pr_err("%s: tf700_irda_tx Direction configuration failed %d\n",
							__func__, ret);
		gpio_set_value(CARDHU_IRDA_SD, 0);
		goto closure;
	}
...
static int SD_config(void)
{
	...
	if (ret < 0) {
		pr_err("%s: tf700_irda_sd gpio request failed %d\n",
							__func__, ret);
		return ret;
	}
	...
	if (ret)
		pr_err("%s: tf700_irda_sd Direction configuration failed %d\n",
						__func__, ret);
	return ret;
}

static void tf700_irda_start(void)
{
	FPRINT("Start of the Func %s\n", __func__);
...
static void tf700_irda_shutdown(void)
{
	FPRINT("Start of the Func %s\n", __func__);
...
static int tf700_irda_init(void)
{
	int ret = 0;
...
static void tf700_irda_remove(void)
{
	FPRINT("Start of the Func %s\n", __func__);
...
struct tegra_uart_platform_data tf700_irda_pdata = {
	.is_irda		= true,
	.irda_init		= tf700_irda_init,
	.irda_start		= tf700_irda_start,
	.irda_mode_switch	= tf700_irda_mode_switch,
	.irda_shutdown		= tf700_irda_shutdown,
	.irda_remove		= tf700_irda_remove,
};
