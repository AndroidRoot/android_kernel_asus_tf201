...
static int tf700_wifi_status_register(void (*callback)(int , void *), void *);

static int tf700_wifi_reset(int on);
static int tf700_wifi_power(int on);
static int tf700_wifi_set_carddetect(int val);

static struct wifi_platform_data tf700_wifi_control = {
	.set_power	= tf700_wifi_power,
	.set_reset	= tf700_wifi_reset,
	.set_carddetect	= tf700_wifi_set_carddetect,
};
...
static struct platform_device broadcom_wifi_device = {
	...
	.dev		= {
		.platform_data = &tf700_wifi_control,
	},
};
...
static struct tegra_sdhci_platform_data tegra_sdhci_platform_data2 = {
	.mmc_data = {
		.register_status_notify	= tf700_wifi_status_register,
#ifdef CONFIG_MMC_EMBEDDED_SDIO
...
static int tf700_wifi_status_register(
...
static int tf700_wifi_power(int on)
{
	 ...
	 *
	 * tf700 GPIO WLAN enable is part of SDMMC3 pin group
	 */
...
static int __init tf700_wifi_init(void)
{
	int rc;
...
static int __init tf700_wifi_prepower(void)
{
	if (!machine_is_tf700())
		return 0;
...
subsys_initcall_sync(tf700_wifi_prepower);
#endif

int __init tf700_sdhci_init(void)
{
