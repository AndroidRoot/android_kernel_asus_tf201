/*
 * arch/arm/mach-tegra/board-tf300-misc.c
 *
...
unsigned int tegra3_query_touch_module_pcbid(void)
{
	...
	touch_pcbid = (HW_DRF_VAL(TEGRA3_DEVKIT, MISC_HW, TOUCHL, tf300_pcbid)) +
		(HW_DRF_VAL(TEGRA3_DEVKIT, MISC_HW, TOUCHH, tf300_pcbid) << 1);
...
unsigned int tegra3_query_audio_codec_pcbid(void)
{
	...
	codec_pcbid = HW_DRF_VAL(TEGRA3_DEVKIT, MISC_HW, ACODEC, tf300_pcbid);
...
unsigned int tegra3_query_pcba_revision_pcbid(void)
{
	...
	ret = HW_DRF_VAL(TEGRA3_DEVKIT, MISC_HW, REVISION, tf300_pcbid);
...
unsigned int tegra3_query_wifi_module_pcbid(void)
{
	...
	wifi_pcbid = HW_DRF_VAL(TEGRA3_DEVKIT, MISC_HW, WIFI, tf300_pcbid);
	ret = wifi_pcbid;
...
static ssize_t tf300_chipid_show(struct kobject *kobj,
...
static ssize_t tf300_pcbid_show(struct kobject *kobj,
	struct kobj_attribute *attr, char *buf)
{
	char *s = buf;
	int i;
	...
	for (i = TEGRA3_DEVKIT_MISC_PCBID_NUM; i > 0; i--) {
		s += sprintf(s, "%c", tf300_pcbid & (1 << (i - 1)) ? '1' : '0');
	}
	s += sprintf(s, "b\n");
	return (s - buf);
}

static ssize_t tf300_projectid_show(struct kobject *kobj,
        struct kobj_attribute *attr, char *buf)
{
        char *s = buf;
...
static ssize_t tf300_projectname_show(struct kobject *kobj,
        struct kobj_attribute *attr, char *buf)
{
        char *s = buf;
...
CARDHU_MISC_ATTR(tf300_chipid);
CARDHU_MISC_ATTR(tf300_pcbid);
CARDHU_MISC_ATTR(tf300_projectid);
CARDHU_MISC_ATTR(tf300_projectname);

static struct attribute *attr_list[] = {
	&tf300_chipid_attr.attr,
	&tf300_pcbid_attr.attr,
	&tf300_projectid_attr.attr,
	&tf300_projectname_attr.attr,
	NULL,
};
...
static struct platform_device *tf300_misc_device;
...
static int pcbid_init(void)
{
	...
	tf300_pcbid = (gpio_get_value(TEGRA_GPIO_PK3) << 8) |
...
int __init tf300_misc_init(void)
{
	...
	tf300_misc_device = platform_device_alloc("tf300_misc", -1);

        if (!tf300_misc_device) {
		ret = -ENOMEM;
		goto fail_platform_device;
        }

	// add a platform device to device hierarchy
	ret = platform_device_add(tf300_misc_device);
	if (ret) {
		pr_err("[MISC]: cannot add device to platform.\n");
		goto fail_platform_add_device;
	}

	ret = sysfs_create_group(&tf300_misc_device->dev.kobj, &attr_group);
	if (ret) {
...
fail_sysfs:
	platform_device_del(tf300_misc_device);

fail_platform_add_device:
	platform_device_put(tf300_misc_device);
