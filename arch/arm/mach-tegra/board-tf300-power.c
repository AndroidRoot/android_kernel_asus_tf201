/*
 * arch/arm/mach-tegra/board-tf300-power.c
 *
...
static struct i2c_board_info __initdata tf300_regulators[] = {
	{
...
int __init tf300_regulator_init(void)
{
	...
	if (pmu_board_info.board_id == BOARD_PMU_PM298)
		return tf300_pm298_regulator_init();

	if (pmu_board_info.board_id == BOARD_PMU_PM299)
		return tf300_pm299_regulator_init();
	...
	i2c_register_board_info(4, tf300_regulators, 1);
...
int __init tf300_fixed_regulator_init(void)
{
	struct board_info board_info;
	struct board_info pmu_board_info;
	...
	if (pmu_board_info.board_id == BOARD_PMU_PM298)
		return tf300_pm298_gpio_switch_regulator_init();

	if (pmu_board_info.board_id == BOARD_PMU_PM299)
		return tf300_pm299_gpio_switch_regulator_init();
...
subsys_initcall_sync(tf300_fixed_regulator_init);

static void tf300_board_suspend(int lp_state, enum suspend_stage stg)
{
	if ((lp_state == TEGRA_SUSPEND_LP1) && (stg == TEGRA_SUSPEND_BEFORE_CPU))
		tegra_console_uart_suspend();
}

static void tf300_board_resume(int lp_state, enum resume_stage stg)
{
	if ((lp_state == TEGRA_SUSPEND_LP1) && (stg == TEGRA_RESUME_AFTER_CPU))
		tegra_console_uart_resume();
}

static struct tegra_suspend_platform_data tf300_suspend_data = {
	.cpu_timer	= 2000,
	.cpu_off_timer	= 200,
	...
	.board_suspend = tf300_board_suspend,
	.board_resume = tf300_board_resume,
};

int __init tf300_suspend_init(void)
{
	struct board_info board_info;
	struct board_info pmu_board_info;
	...
	if ((pmu_board_info.fab == BOARD_FAB_A03) ||
		...
		tf300_suspend_data.corereq_high = true;
	...
	case BOARD_E1256:
	case BOARD_E1257:
		tf300_suspend_data.cpu_timer = 5000;
		tf300_suspend_data.cpu_off_timer = 5000;
		break;
	default:
		break;
	}

	tegra_init_suspend(&tf300_suspend_data);
	return 0;
}

#ifdef CONFIG_TEGRA_EDP_LIMITS

int __init tf300_edp_init(void)
{
...
static char *tf300_battery[] = {
	"bq27510-0",
};

static struct gpio_charger_platform_data tf300_charger_pdata = {
	.name = "ac",
	.type = POWER_SUPPLY_TYPE_MAINS,
	.gpio = AC_PRESENT_GPIO,
	.gpio_active_low = 0,
	.supplied_to = tf300_battery,
	.num_supplicants = ARRAY_SIZE(tf300_battery),
};

static struct platform_device tf300_charger_device = {
	.name = "gpio-charger",
	.dev = {
		.platform_data = &tf300_charger_pdata,
	},
};

static int __init tf300_charger_late_init(void)
{
	if (!machine_is_tf300())
		return 0;

	/* platform_device_register(&tf300_charger_device); */
	return 0;
}

late_initcall(tf300_charger_late_init);
