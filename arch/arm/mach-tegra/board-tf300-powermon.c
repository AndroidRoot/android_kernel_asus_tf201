/*
 * arch/arm/mach-tegra/board-tf300-powermon.c
 *
...
static struct i2c_board_info tf300_i2c0_ina219_board_info[] = {
	[INA_I2C_ADDR_40] = {
...
int __init tf300_pmon_init(void)
{
	...
	if (bi.fab == BOARD_FAB_A04) {
		tf300_i2c0_ina219_board_info[INA_I2C_ADDR_44].platform_data =
			&power_mon_info[UNUSED_RAIL];
		tf300_i2c0_ina219_board_info[INA_I2C_ADDR_4A].platform_data =
			&power_mon_info[VDD_CORE_IN];
	}

	if (bi.board_id != BOARD_PM269) {
		i2c_register_board_info(0, tf300_i2c0_ina219_board_info,
			ARRAY_SIZE(tf300_i2c0_ina219_board_info));
	}
